...
export default class UsersList extends Component {
    ...
    fetchProducts(){
        ...
        axios.post('http://localhost:4000/dispatch_product',find_vendorproduct)
             .then(response => {
    ...
    onSubmit(event,product_id) {
        event.preventDefault();
        const orders_dispatched = {
            product_id: product_id,
            vendor_email:localStorage.getItem("emaillogged")
        }
        console.log(orders_dispatched)
        axios.post('http://localhost:4000/dispatch_order',orders_dispatched)
             .then(res => {
                this.fetchProducts()
                })
                        ...
                        this.state.products.map((currentUser, i) => {
                            return (
                                <tr>
                                    ...
                                    <td><button onClick = {(event) => this.onSubmit(event, currentUser.product_id)}>Dispatch Order</button></td>
